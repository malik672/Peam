...
fn root_hex(root: Bytes32) -> String {
...
fn file_store_roundtrip_persists_data_and_meta() {
    ...
    assert_eq!(reopened.get_state(&state_root), Some(&state));
...
fn file_store_put_signed_block_persists_chain_heads() {
    ...
    assert_eq!(reopened.get_block_by_slot(1), Some(&signed.message.block));
    assert_eq!(reopened.head(), Some(root));
...
fn file_store_skips_corrupt_entries_and_reports_recovery() {
    ...
    // Invalid root filename + invalid payload should both be skipped.
    std::fs::write(dir.join("blocks").join("invalid_root.ssz"), [0x01, 0x02])
        .expect("write corrupt block");
    std::fs::write(
        dir.join("states")
...
fn file_store_persists_slot_indexes() {
    ...
    assert!(dir.join("state_index.txt").exists());
    assert!(dir.join("block_index.txt").exists());
...
fn prune_keeps_head_justified_finalized() {
    ...
    assert!(store.get_block(&old_root_c).is_none());
...
fn prune_removes_old_noncanonical_entries() {
    ...
    assert!(report.removed_signed_blocks >= 1);

    assert!(store.get_state(&old_state_root).is_none());
    assert!(store.get_state(&new_state_root).is_some());
    assert!(store.get_block(&old_block_root).is_none());
    assert!(store.get_block(&new_block_root).is_some());
    assert!(store.get_signed_block(&old_signed_root).is_none());
...
fn prune_rewrites_indexes_consistently() {
    ...
    let state_index = std::fs::read_to_string(dir.join("state_index.txt")).expect("state index");
    let block_index = std::fs::read_to_string(dir.join("block_index.txt")).expect("block index");
    assert!(!state_index.contains("5="));
    assert!(state_index.contains("44="));
    assert!(!block_index.contains("7="));
    assert!(block_index.contains("45="));
...
fn prune_persists_across_reopen() {
    ...
    let reopened = FileStore::open(&dir).expect("reopen file store");
    assert!(reopened.get_block(&old_root).is_none());
    assert_eq!(reopened.get_block(&new_root), Some(&new_block));
...
fn interrupted_temp_file_does_not_break_reopen() {
    ...
    std::fs::write(dir.join("blocks").join(".block.ssz.tmp.interrupted"), [0xAB, 0xCD])
        .expect("write temp blob");

    let reopened = FileStore::open(&dir).expect("reopen file store");
    assert_eq!(reopened.get_block(&root), Some(&block));
    let report = reopened.recovery_report();
    assert!(report.skipped_unknown_version >= 1);
...
fn truncated_blob_recovery_after_simulated_crash() {
    ...
    // Simulate interrupted write that left a truncated .ssz file at the canonical path.
    let path = dir.join("blocks").join(format!("{}.ssz", root_hex(root)));
    std::fs::write(path, [0x01, 0x02, 0x03, 0x04]).expect("truncate canonical blob");
...
fn write_failure_on_one_key_does_not_corrupt_existing_entries() {
    ...
    // Force rename failure for this key by creating a directory at the target file path.
    let bad_root = Bytes32::from([0x54u8; 32]);
    let bad_path = dir.join("states").join(format!("{}.ssz", root_hex(bad_root)));
    std::fs::create_dir_all(&bad_path).expect("create blocking directory");

    let mut store = FileStore::open(&dir).expect("reopen file store");
    ...
    assert_eq!(reopened.get_state(&good_root), Some(&good_state));
